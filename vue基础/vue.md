...
# 初识路由 
	...
	路由即路径
	路由分两种 一种是前端路由,该路由不会刷新页面，一种是后端路，该路由会刷新页面

	如何安装Router
	vue 是渐进式 所以安装vue 只需要vue add router 
	之后会询问你使用哪几种模式
	vue 有两种模式 一种是hash 模式 一种是历史模式 都不会刷新页面

	这里再区分一下 router  和 route 
	router：指代路由器
	route： 路线
...
# vue文件里怎么引入样式 
	...
	注意箭头函数的区别
	a=>b 和a = >{ b };
	前者返回b 语句  后者执行b语句 返回undefined  使用场景在于vue-router配置其他
	routes的时候使用
